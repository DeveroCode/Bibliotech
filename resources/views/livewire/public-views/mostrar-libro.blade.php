<div class="p-5 lg:px-40 bg-white">
    <article class="flex flex-col md:flex-row md:overflow-hidden my-14">
        <div class="max-w-md p-4 md:order-2">
            <img alt="Imagen {{ $libro->titulo }}" class="mx-auto object-contain object-center shadow-sm"
                src="{{ asset('storage/libros/' . $libro->imagen) }}" />

            <div class="ml-3 mt-2 md:mt-[75px] md:ml-0">
                <span class="mt-10">
                    <i class="fa-solid fa-circle-exclamation"></i>
                    Estante:
                    <span> {{ $libro->estante->estante }}</span>
                </span>
            </div>
        </div>
        ...
        <div class="p-4 sm:p-6 w-full md:w-2/3 lg:w-3/5 lg:mx-auto md:order-1 flex flex-row md:flex-col">
            <div class="md:px-10">
                <div class="text-center md:text-start">
                    ...
                    <span class="font-extrabold text-gray-500 capitalize">Por {{ json_decode($libro->autores)[0]->autor
                        }}</span>
                ...
                <div class="flex flex-col lg:flex-row lg:justify-between">
                    <div>
                        <button onclick="goBack()" class="inline-flex items-center px-10 py-2 bg-purple-600
                    ...
                    <div class="mt-3">
                        ...
                        @if ($libro->cantidad <= 10) <span
                            ...
                            @else
                            <span
                                class="inline-flex items-center gap-1 rounded-full bg-green-100 px-2 py-1 text-xs font-semibold text-green-600 ml-2">
                ...
                <div class="flex flex-col items-center swiper-slide max-w-sm m-4">
                    <div class="w-28 h-32 absolute z-10">
                        <img class="object-cover w-full h-full"
                            src="{{ asset('storage/libros/' . $relacionado->imagen) }}"
                            alt="Imagen {{ $libro->titulo }}">
                    </div>
                    <div class="bg-gray-100 rounded-lg shadow-md p-5 h-44 relative z-0 mt-28">
                        <div class="h-24">
                            <h5 class="text-gray-900 font-bold text-lg capitalize tracking-tight mb-2 dark:text-white">
                                {{ Str::limit($relacionado->titulo, 31) }}</h5>
                        ...
                        <div class="h-20 py-2">
                            <a href="{{ route('show.books', $relacionado->id) }}"
                                class="text-white bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-3 py-2 text-center inline-flex items-center  dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                Mas información
